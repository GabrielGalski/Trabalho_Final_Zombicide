...
public class Engine extends JPanel implements Serializable {
    ...
    private JButton createButton(int i, int j) {
        ...
        // Configura a aparência inicial usando o método de Cell
        cell.configureButton(button, false, false, debugMode); // Inicialmente invisível, ajustado por updateVisibility
    ...
    private void movePlayer(int destX, int destY) {
        ...
        if ((dx == 1 && dy == 0) || (dx == 0 && dy == 1)) { // Movimento válido (adjacente)
            if (destX >= 0 && destX < TAM && destY >= 0 && destY < TAM) {
                ...
                if (inCombat) {
                    ...
                    if (zombieDamage == 0) {
                        zombieDamage = 1; // Dano garantido mínimo
                    }
    ...
    private void tryMoveZombie(int x, int y, Entity zombie, int maxSteps) {
        ...
        for (int step = maxSteps; step > 0; step--) {
            ...
            if (isWithinBoard(newX, newY)) {
                ...
                char targetType = cells[newX][newY].getType();
                if (targetType == 'B') {
                    int jumpX = x + stepX * 2;
                    int jumpY = y + stepY * 2;
                    if (isWithinBoard(jumpX, jumpY) && cells[jumpX][jumpY].getType() == 'V') {
                        if (isOriginalPositionChest) {
                            cells[x][y] = new Chest();
                        } else {
                            cells[x][y] = Fixed.createFixed('V');
                        }
                        ((Mobile) zombie).move(jumpX - x, jumpY - y);
                        cells[jumpX][jumpY] = zombie;
                        if (Math.abs(jumpX - playerX) + Math.abs(jumpY - playerY) <= 1) {
                            startCombat(jumpX, jumpY);
                        }
                        return;
                    }
                    continue;
                ...
                if (step == 2) {
                    int interX = x + (horizontalPriority ? stepX : 0);
                    int interY = y + (horizontalPriority ? 0 : stepY);
                    if (cells[interX][interY].getType() == 'B') {
                        int jumpX = x + stepX * 2;
                        int jumpY = y + stepY * 2;
                        if (isWithinBoard(jumpX, jumpY) && cells[jumpX][jumpY].getType() == 'V') {
                            if (isOriginalPositionChest) {
                        ...
                        continue;
                    } else if (!isPassable(interX, interY)) {
                        continue;
                    }
                }
                ...
                if (isPassable(newX, newY)) {
                    ...
                    ((Mobile) zombie).move(newX - x, newY - y);
                    cells[newX][newY] = zombie;
                    if (Math.abs(newX - playerX) + Math.abs(newY - playerY) <= 1) {
